...
const connectDB = async () => {
    try {
        await mongoose.connect(process.env.DATABASE || app.config.database, {
            useNewUrlParser: true,
            useUnifiedTopology: true,
        });
        app.logger.log('MongoDB', 'Connected to database');
        handlePendingDeletions();
    } catch (err) {
        app.logger.error('MongoDB', `Connection error: ${err.message}`);
        process.exit(1);
    }
};
