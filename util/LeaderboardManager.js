...
function LeaderboardManager(app) {
    var manager = {
        ...
        update: async function() {
            ...
            var pixelCounts = {};
            try {
                for await (const pixel of Pixel.find({lastModified: {$gt: dateBackLastWeek}}, {editorID: 1}).cursor()) {
                ...
                const users = await User.find({_id: { $in: m.topUsers }}, {_id: 1, banned: true, deactivated: true});
